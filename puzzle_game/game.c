...
{
    int left = MISSING_MIN_X;
    int mid = (WINDOW_W - MISSING_W) / 2;
    int right = WINDOW_W - MISSING_W - MISSING_MIN_X;
    int top = MISSING_MIN_Y;
    int bottom = TRAY_Y - MISSING_H - MISSING_MIN_Y;
    int candidates[6][2] = {
        {left,  top},
        {mid,   top},
        {right, top},
...
int game_init(GameContext *ctx)
{
    ...
    ctx->font_large = TTF_OpenFont(FONT_PATH, 20);
    ctx->font_small = TTF_OpenFont(FONT_PATH, 11);
    if (!ctx->font_large || !ctx->font_small) {
        ctx->font_large = TTF_OpenFont("/usr/share/fonts/truetype/dejavu/DejaVuSansMono-Bold.ttf", 20);
        ctx->font_small = TTF_OpenFont("/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf", 11);
    }
    if (!ctx->font_large || !ctx->font_small) {
...
static void clamp_piece_to_screen(Piece *piece)
{
    if (piece->drag_rect.x < 0) piece->drag_rect.x = 0;
    if (piece->drag_rect.y < 0) piece->drag_rect.y = 0;
    if (piece->drag_rect.x + piece->drag_rect.w > WINDOW_W)
        piece->drag_rect.x = WINDOW_W - piece->drag_rect.w;
    if (piece->drag_rect.y + piece->drag_rect.h > WINDOW_H)
        piece->drag_rect.y = WINDOW_H - piece->drag_rect.h;
}
...
static void try_place_piece(GameContext *ctx, int idx)
{
    int pct;

    if (idx < 0 || idx >= 3 || ctx->pieces[idx].placed)
        return;

    pct = rect_overlap_pct(&ctx->target_rect, &ctx->pieces[idx].drag_rect);
    if (pct >= 70) {
...
void game_handle_event(GameContext *ctx, const SDL_Event *e)
{
    if (e->type == SDL_KEYDOWN) {
        ...
        if (ctx->state == STATE_PLAYING) {
            const int step = 10;
            Piece *selected = &ctx->pieces[ctx->selected_index];

            if (e->key.keysym.sym == SDLK_a || e->key.keysym.sym == SDLK_PAGEUP)
                choose_next_piece(ctx, -1);
            else if (e->key.keysym.sym == SDLK_d || e->key.keysym.sym == SDLK_PAGEDOWN)
                choose_next_piece(ctx, 1);
            else if (e->key.keysym.sym == SDLK_LEFT)
                selected->drag_rect.x -= step;
            else if (e->key.keysym.sym == SDLK_RIGHT)
                selected->drag_rect.x += step;
            else if (e->key.keysym.sym == SDLK_UP)
                selected->drag_rect.y -= step;
            else if (e->key.keysym.sym == SDLK_DOWN)
                selected->drag_rect.y += step;
            else if (e->key.keysym.sym == SDLK_SPACE || e->key.keysym.sym == SDLK_RETURN)
                try_place_piece(ctx, ctx->selected_index);

            clamp_piece_to_screen(selected);
        }
        return;
    }
...
void game_render(GameContext *ctx)
{
    ...
    if (ctx->font_small && ctx->state == STATE_PLAYING) {
        ...
        SDL_Surface *s = TTF_RenderText_Blended(ctx->font_small, "ARROWS MOVE  A/D SELECT  SPACE PLACE", col);
        if (s) {
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < 3; i++) {
            ...
            if (ctx->state == STATE_PLAYING && i == ctx->selected_index) {
                SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
                SDL_SetRenderDrawColor(r, 255, 230, 70, 230);
