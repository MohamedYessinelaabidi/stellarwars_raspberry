...
int initSDL(SDL_Window **window, SDL_Renderer **renderer)
{
    ...
    /* Raspberry Pi 4 + 3.5" 480×320 display:
     * Use FULLSCREEN_DESKTOP so SDL stretches to the framebuffer without
     * requiring an exact mode match.  The logical size set below keeps all
     * game coordinates in SCREEN_W×SCREEN_H space regardless of the real
    ...
    *renderer = SDL_CreateRenderer(*window, -1,
    ...
    /* Lock all rendering to 480×320 logical pixels – SDL scales to the
     * actual framebuffer size automatically. */
